...
if __name__ == "__main__":
    ...
    parser.add_argument("--outfile_prefix", type=str, default="out",
                        help="A prefix for all of the outfiles produced by the program in whichever mode you run it in. ")
    parser.add_argument("--output_subtrees", action="store_true",
                        help="Will output subtrees analyzed to a file that has the gene name followed by .subtree.")
    parser.add_argument("--annotated_tree", type=str,
                        help="File containing a gene tree labelled by 'r' mode. Required in 'summarize' mode.")
    parser.add_argument("--perfect_concordance", action="store_true",
                        help="If this argument is used in normal mode, the program will give a list of all the trees in the input folder that contain 0 conflicts with the species tree.")
    parser.add_argument("--perfect_concordance_strict", action="store_true",
                        help="Same as perfect_concordance, except that only gene trees containing every taxon within the species tree will be included.")
...
this mode.")
        ...
        for file in file_list:
            ...
            for line in gene_file:
                ...
                if len(trees) == 1:
                    ...
                    trarray.append(g)
                    # final tree has all the names anyway
                    extra_names.append([])
        ...
        concord_gene_out = open(str(outfile_prefix) +
                                "_concord_gene_counts.tre", "w")
        conflict_gene_out = open(
            str(outfile_prefix) + "_conflict_gene_counts.tre", "w")
        if cutoff == 0:
            nodes_analyzable_out = open(
                str(outfile_prefix) + "_total_analyzed.tre", "w")
        ...
        concord_gene_dup_out = str(outfile_prefix) + \
            "_multi_concord_gene_counts.csv"
        make_trees.clear_labels(species_root)
        make_trees.tree_map3(species_root, total_concordances,
                             concord_gene_list, concord_gene_dup_out)
        make_trees.add_zeros(species_root)
        ...
        conflict_gene_dup_out = str(
            outfile_prefix) + "_multi_conflict_gene_counts.csv"
        make_trees.clear_labels(species_root)
        make_trees.tree_map3(species_root, total_conflicts,
                             conflict_gene_list, conflict_gene_dup_out)
        make_trees.add_zeros(species_root)
...
mode.")
        ...
        for tree in trarray:
            ...
            # All the other subtrees can be analysed.
                ...
                rel_list = comparisons.comp_biparts(
                    subtree_biparts, species_biparts, subtree_taxa, species_name_array, "some_log_name", 0, "r", "")
                ...
                if outfile_subtrees:
                    ...
                    if len(extra_names[count]) != 0:
                        out_subs.write(
                            "((" + ",".join(extra_names[count]) + ")," + out_mrca.get_newick_repr() + ");\n")
                    else:
    ...
    # Looking for all gene trees with exact concordance to a given bipart.
        ...
        for filename in file_list:
            ...
            count += 1
            total_concordances = []
            sys.stderr.write("processing " + str(count) +
                             " of " + str(len(file_list)) + '\r')
            ...
            for tree in trarray:
                ...
                # Searching for patterns concordant with the query.
                    ...
                    if query_remove:
                        ...
                        # and the query.
                        check = any(
                            item in query_entered for item in extra_names[count2])
                        if check:
                            pass
                        ...
                        # items in the query that are not in this subtree.
                            ...
                            not_in_removable, not_in_subtree = comparisons.unique_array(
                                removable, subtree_taxa)
                            search_list = [
                                x for x in query_entered if x not in not_in_subtree]
                    else:
        ...
        if outfile_subtrees:
            for x in range(0, len(subtree_printout)):
                ...
                t = printout_filename[x] + ": " + \
                    subtree_printout[x].get_newick_repr(showbl=True)
                outw.write(t+"\n")
            outw.close()
        ...
        for x in range(0, len(final_print_names)):
            outw.write(str(final_print_names[x]) +
                       "," + str(final_print_matches[x])+"\n")
            total_hits += int(final_print_matches[x])
        outw.close()
        print("Total gene families: " + str(len(final_print_names)) +
              " Total matches: " + str(total_hits))
    ...
    # If query_bipart and _species_tree are provided, ...?
        ...
        counts = []
        possible = ["Duplications", "D", "Concordances",
                    "*", "Conflicts", "X", "Uninformative", "U"]
        ...
        if query_bipart and species_tree:
            ...
            trarray = []
            extra_names = []
            make_trees.subtree_divide_at_base_of_dup(
                gene_root, trarray, extra_names)

            count2 = -1
            outfile_sum.write(
                "LeftTips\tLeftDuplications\tLeftConcord\tLeftConflict\tLeftUninformative\tRightTips\tRightDuplications\tRightConcord\tRightConflict\tRightUninformative\n")
            for tree in trarray:
                count2 += 1
                ...
                if tree.istip == False and tree.label == "D":
                    ...
                    if check:
                        ...
                        sp_dup_node = []
                        sp_dup_node = comparisons.get_smallest_match(
                            bipart_array, ingroup)
                        ...
                        if sorted(query_entered) == sorted(sp_dup_node):
                            ...
                            outfile_sum.write(str(len(tree.children[0].lvsnms()))
                                              ...
                                              + "\t" +
                                              str(len(
                                                  tree.children[1].lvsnms()))
                                              + "\t" +
                                              str(child2_counts.count(
                                                  possible[1]))
                                              + "\t" +
                                              str(child2_counts.count(
                                                  possible[3]))
            ...
            print(
                "For more detailed analysis give species tree and a bipartition of interest")
...
this mode.")
        ...
        sortaout.write(
            "Filename,Concordances,Root-Tip-Variance,TreeLength,TotalTips\n")
        ...
        # This will act on trees with no duplications (orthologs).
                ...
                sortaout.write(str(file)+","+str(len(concordances))+"," +
                               str(RT_var)+","+str(t_len)+","+str(len(gene_name_array))+"\n")
    else:
