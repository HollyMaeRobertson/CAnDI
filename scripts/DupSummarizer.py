...
def preorder(root, HASH, tip_HASH, len_HASH, Parent_HASH, Species_HASH, counter, count_array):
    ...
    for child in root.children:
        ...
        if child.label == "D":
            ...
            species_bipart = []
            species_bipart = check_corresponding_species_bipart(
                tip_aa, bipart_array)
        ...
        preorder(child, HASH, tip_HASH, len_HASH, Parent_HASH,
                 Species_HASH, counter, count_array)
...
for i in name2:
    ...
    preorder(n, HASH, tip_HASH, len_HASH, Parent_HASH,
             Species_HASH, counter, count_array)
    ...
    for x in HASH:
        ...
        if x in Parent_HASH:
            outw.write("Duplication" + str(x) + "\t" + str(Parent_HASH[x]) + "\t" + str(Left_Duplication) + "\t" + str(Right_Duplication) + "\t" + str(DupDiff)
                       + "\t" + str(Left_Conflict) + "\t" +
                       str(Right_Conflict) + "\t" + str(ConfDiff) + "\t"
                       + str(Left_Concordance) + "\t" +
                       str(Right_Concordance) + "\t" + str(ConcDiff) + "\t"
                       + str(Left_Uninformative) + "\t" +
                       str(Right_Uninformative) + "\t" + str(UnDiff) + "\t"
                       + str(Left_Tips) + "\t" + str(Right_Tips) +
                       "\t" + str(TipDiff) + "\t" + str(Left_Sum) + "\t"
